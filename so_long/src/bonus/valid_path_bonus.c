...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/22 17:31:43 by okirca            #+#    #+#             */
/*   Updated: 2023/08/22 17:35:12 by okirca           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (cpy->map[pl_x][pl_y] != '1')
	{
		if (cpy->map[pl_x][pl_y] != 'E')
		{
			if (cpy->map[pl_x][pl_y] == 'C')
				cpy->collect_c--;
			cpy->map[pl_x][pl_y] = 'X';
			if (cpy->map[pl_x][pl_y + 1] != '1' &&
			cpy->map[pl_x][pl_y + 1] != 'X')
				ft_valid_path(cpy, pl_x, pl_y + 1);
			if (cpy->map[pl_x][pl_y - 1] != '1' &&
			cpy->map[pl_x][pl_y - 1] != 'X')
				ft_valid_path(cpy, pl_x, pl_y - 1);
			if (cpy->map[pl_x - 1][pl_y] != '1' &&
			cpy->map[pl_x - 1][pl_y] != 'X')
